...
export const seedServiceConfigItems = async (): Promise<void> => {
  ...
  try {
    ...
    for (const item of configItems) {
      ...
      const existing = await query(
        `SELECT service_config_item_id FROM courier_service_config_items 
         WHERE courier_service_id = $1 AND name = $2`,
        [fedexServiceId, item.name]
      );
      ...
      const result = await query(
        `INSERT INTO courier_service_config_items (
