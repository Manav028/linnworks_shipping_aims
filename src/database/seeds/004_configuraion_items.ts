...
export const seedConfigurationItems = async (): Promise<void> => {
  console.log('Seeding configuration items...');

  try {
    // Fetch stage IDs
    const stagesResult = await query(
      `SELECT config_stage_id, stage_name FROM configuration_stages`
    );

    const stageMap = new Map(
      stagesResult.rows.map((r: any) => [r.stage_name, r.config_stage_id])
    );
    ...
    for (const item of items) {
      ...
      const exists = await query(
        `SELECT 1 FROM configuration_items 
         WHERE config_item_identifier = $1 AND config_stage_id = $2`,
        [item.identifier, stageId]
      );
